...
uniform sampler2D normalMap, diffuseMap;
...
void main()
{
    ...
    PointLight( 0, eye, wc3Position, normalize( newNormal ), amb, diff, spec, atten );
	
	color += amb * gl_FrontMaterial.ambient +
    diff * gl_FrontMaterial.diffuse * texture2D( diffuseMap, gl_TexCoord[0].st );
